...
// Brackets in link text and parens/spaces in URLs break Markdown links —
// escape/encode them so a title like "Attention [v2] (revised)" survives.
const mdText = (s) => String(s ?? '').replace(/([[\]])/g, '\\$1');
const mdUrl = (u) => String(u ?? '').replace(/\)/g, '%29').replace(/ /g, '%20');

const linkLine = (p) => {
  const parts = [];
  if (p.links?.pdf) parts.push(`[PDF](${mdUrl(p.links.pdf)})`);
  if (p.links?.arxiv) parts.push(`[arXiv](${mdUrl(p.links.arxiv)})`);
  if (p.links?.doi) parts.push(`[DOI](${mdUrl(p.links.doi)})`);
  if (p.links?.landing) parts.push(`[Page](${mdUrl(p.links.landing)})`);
  return parts.join(' · ');
};
...
const titleMd = (p) => {
  ...
  return link ? `[${mdText(p.title)}](${mdUrl(link)})${star}` : `${p.title}${star}`;
};
...
export function renderMarkdown(enriched) {
  ...
  const path = Array.isArray(enriched.readingPath) ? enriched.readingPath : [];
  if (path.length) {
    out.push('## Reading path', '', '_New to this topic? Read in this order._', '');
    let n = 0;
    for (const s of path) {
      const p = byId.get(s.id);
      if (!p) continue;
      out.push(`${++n}. **${titleMd(p)}** — ${s.why || ''}`);
    }
    out.push('');
  }
