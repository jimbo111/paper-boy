  ...
  return {
    ...
    async complete({ prompt = '', schema } = {}) {
      ...
      if (req.includes('readingPath')) {
        // Echo back real ids from the prompt so the path always maps to real papers.
        const ids = [...prompt.matchAll(/^- (\S+):/gm)].map((m) => m[1]);
        data.readingPath = ids.slice(0, 3).map((id) => ({ id, why: 'A reasonable next step for this topic.' }));
      }
