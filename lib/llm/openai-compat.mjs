...
// `tokenParam` names the output-cap field: classic servers take `max_tokens`, but
// newer OpenAI models (o-series, gpt-5 family) reject it and require
// `max_completion_tokens` — the openai preset opts in via this knob.
export function makeProvider({ name, defaultModel, defaultBase, tokenParam = 'max_tokens' }) {
  return {
    name,
    defaultModel,
    ...
    buildRequest({ model, apiKey, baseUrl, system, prompt, schema, maxTokens = 4096 }) {
      ...
      const body = { model, [tokenParam]: maxTokens, messages };
