...
function makeLimiter({ rps = 2, maxConcurrency = 4 } = {}) {
  let inFlight = 0;
  let nextStart = 0; // earliest time the next job may begin
  let timer = null; // single pending timer — prevents concurrent enqueues double-starting
  const queue = [];
  const minGap = rps > 0 ? 1000 / rps : 0;
  const pump = () => {
    if (timer || !queue.length || inFlight >= maxConcurrency) return;
...
export function makeClient({ provider, model, apiKey, baseUrl, rps, maxConcurrency,
  ...
  async function once({ system, prompt, schema, signal, maxTokens }) {
    ...
    // Completions are much slower than the 15s transport default (deep-dive prompts
    // carry up to 60k chars of full text) — give the model time to answer.
    const res = await postImpl(req.url, { headers: req.headers, body: req.body, signal, timeout: 120000 });
    if (!res || !res.ok) {
    ...
    let text, stopReason;
    try { ({ text, stopReason } = provider.parseResponse(parsedBody)); }
    catch (err) { return { ok: false, status: res.status, error: String(err.message || err), raw: res.body }; }
    return { ok: true, text, stopReason, raw: res.body };
  }

  // Output cut off at the token ceiling can never parse as JSON — re-prompting
  // with the same ceiling is guaranteed to truncate again.
  const truncated = (r) => r.stopReason === 'max_tokens' || r.stopReason === 'length';
  ...
  async function complete({ system, prompt, schema, signal, maxTokens } = {}) {
    return limit(async () => {
      ...
      for (let attempt = 0; attempt <= maxRepairRetries; attempt++) {
        ...
        // On a repair retry, hand the model its own bad output and demand clean
        // JSON. After a truncation, resend the original prompt instead — the reply
        // wasn't malformed, it was cut off, so we raise the ceiling below.
        const p = attempt === 0 || wasTruncated ? prompt
          : `${prompt}\n\nYour previous reply could not be parsed as JSON:\n${truncate(lastText, 1500)}\n\nReturn ONLY valid JSON. No prose, no code fences.`;
        const r = await once({ system, prompt: p, schema, signal, maxTokens: tokens });
        if (!r.ok) {
        ...
        wasTruncated = truncated(r);
        if (wasTruncated) tokens = (tokens || 4096) * 2;
        lastErr = wasTruncated ? `output truncated at the token limit (${j.error})` : j.error;
      }
