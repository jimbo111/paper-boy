...
export function mapOpenAlex(data) {
  ...
  return rows.map((r) => {
    ...
    // OpenAlex `ids` carries no arxiv entry; arXiv-hosted works are identified by
    // their DataCite DOI 10.48550/arxiv.<id>. Recovering the arXiv id here is what
    // lets dedup match this record against the same paper from the arXiv feed.
    const arxivId = (r.ids?.arxiv || '').match(/abs\/(.+)$/)?.[1]
      || doi?.match(/^10\.48550\/arxiv\.(.+)$/)?.[1] || null;
...
export async function fetchOpenAlex(query, opts = {}) {
  ...
  return data ? mapOpenAlex(data) : null; // null = the query itself failed (vs a real empty result)
}
...
// a DOI directly in the path; there is NO `arxiv:` lookup namespace (it 404s), but every
// arXiv paper has the DataCite DOI 10.48550/arxiv.<id>, which resolves fine.
export function workLookupUrl(paper, mailto) {
  ...
  if (paper.arxivId) return `${base}https://doi.org/10.48550/arxiv.${paper.arxivId}?select=id,referenced_works${contactParam(mailto)}`;
  return null;
}
