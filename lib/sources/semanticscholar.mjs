...
export function mapS2(data) {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      // No date fallback from `year` — inventing "Jan 1" would fabricate a date
      // that can win dedup over a real one. `year` alone carries the information.
...
export async function fetchS2(query, opts) {
  ...
  return data ? mapS2(data) : null; // null = the query itself failed (vs a real empty result)
}
