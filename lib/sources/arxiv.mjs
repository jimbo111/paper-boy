...
export function parseAtom(xml) {
  ...
  return entries.map((e) => {
    ...
    // Lazy match anchored at the end so old-style ids containing a "v"
    // (e.g. solv-int/9711008) survive intact; the version suffix is dropped.
    const arxivId = (idUrl.match(/abs\/(.+?)(?:v\d+)?$/) || [])[1] || null;
...
export async function fetchArxiv(query, opts) {
  ...
  return xml == null ? null : parseAtom(xml); // null = the query itself failed (vs a real empty feed)
}
